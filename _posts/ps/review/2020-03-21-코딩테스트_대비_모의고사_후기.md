...
# 1 - 스티커 붙이기
## [아직 문제가 재공개 되지 않았습니다. 재공개 되면 링크가 추가됩니다.] ()
...
5. 그렇게 주어진 k개의 스티커 중 붙일 수 있는걸 다 붙인다면 몇개의 칸에 스티커가 붙었는가?
...
1. 붙일 수 있다면 do_stk()를 통해 붙이고 넘기기,<br>
2. 붙일 수 없다면 turn_stk()를 통해 90도 회전을 시도. 다만 0도, 90도, 180도, 270도 회전하고도 안된다면 그 스티커는 아예 안되니까 넘기기.
...
결과적으로 is_can() 안에서 i, j 일때 flag2 = false로 유지 되었다면, 이는 스티커를 붙일 자리에 스티커가 없다는 의미로 붙일 수 있다는 의미이다.<br>
붙일 수 있다면 해당 자리에 스티커 붙이기 위해 전역변수 canx, cany로 i, j를 넘겨주고 이 함수가 true를 반환 할 수 있도록 하자.<br>
...
스티커를 붙일 수가 없어서 90도를 회전할 경우 함수를 호출한다. 돌리기는 실제로는 자주 할 일이 없는데 A형에서는 기출도 있고 어느정도 그냥 해야하는 테크닉으로 보인다.<br>
...
* 한 큐로 bfs를 돌리기 때문에 시뮬레이션 상 같은 지점에 g, r배양액이 퍼지는 것을 나타 낼 수가 없다.
...
1. 큐를 깐다
1. 지금 지점이 꽃이면 배양액이 지나서 퍼질 수 없기 때문에 스킵한다
2. 4방지점을 탐색하는데 맵 범위 안이면서
...
위 세가지 조건을 만족하면 같은 칸에 같은 시간에 r, g 배양액이 접근한 것으로 보고 꽃을 만든다. (visit[nx][ny].color = 4)<br>
